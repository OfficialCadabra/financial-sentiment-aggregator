...
class BloombergScraper:
    ...
    def scrape(self, start_date, end_date):
        """
        Scrape Bloomberg news articles from RSS feeds.
                ...
                try:
                    ...
                    for item in items:
                        try:
                            ...
                            # Bloomberg often requires a subscription, so we may not get full content
                            # We'll use what we can get from the snippet or summary
                            logger.debug(f"Trying to extract content from {news_url}")
                            
                            article_data = self.extract_article_content(news_url)
                            
                            # If we can't get content, use the snippet
                            if not article_data or not article_data.get('text'):
                                article_data = {'text': snippet, 'summary': snippet}
                    ...
                    logger.error(f"Error parsing RSS feed {feed_url}: XML format error")
    ...
    def extract_article_content(self, url):
        ...
        try:
            ...
            # Skip NLP for Bloomberg since it often has paywalls
            # and we might not get full content
            return {
                'text': article.text,
